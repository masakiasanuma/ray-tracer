...
class TestRT extends RayTracer {
    ...
    constructor (div: HTMLElement) {
        ...
        let sampleSelect = document.getElementById("sample-select") as HTMLSelectElement;
        for (let i = 1; i <= 10; i++) {
          let sampleOpt = document.createElement("option");
          sampleOpt.value = String(i);
          sampleOpt.innerHTML = String(i);
          sampleSelect.append(sampleOpt);
        }

        // Scene selection
        let sceneSelect = document.getElementById("scene-select") as HTMLSelectElement;

        // Check values for jitter and reflection
        let jitterCheck = document.getElementById("jitter") as HTMLInputElement;
        let reflectionCheck = document.getElementById("reflection") as HTMLInputElement;

        // Render scene after setting parameters
        let renderButton = document.getElementById("render");
        renderButton?.addEventListener("click", () => {
          this.set_sample_level(Number(sampleSelect.value));

          if (reflectionCheck.checked) {
            this.reflection_on();
          } else {
            this.reflection_off();
          }

          if (jitterCheck.checked) {
            this.jitter_on();
          } else {
            this.jitter_off();
          }
          ...
          switch (sceneSelect.value) {
            case '0':  this.scene_01(); break;
            case '1':  this.scene_02(); break;
            case '2':  this.scene_03(); break;
            case '3':  this.scene_04(); break;
            case '4':  this.two_spheres_ambient(); break;
            case '5':  this.one_disk(); break;
            case '6':  this.five_disks(); break;
            case '7':  this.three_intersecting_disks(); break;
            case '8':  this.two_spheres_point_light(); break;
            case '9':  this.two_spheres_area_light(); break;
            case '10':  this.colored_lights(); break;
            case '11':  this.three_spheres_disc_area_light(); break;
          }
        });
